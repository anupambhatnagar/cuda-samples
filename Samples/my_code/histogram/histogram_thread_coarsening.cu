...
__global__ void histogram_thread_coarsening(char* data, int* histogram, int length){
...
*/
  ...
  histogram_thread_coarsening <<<grids, blocks>>> (d_input, d_output, N);
  checkCudaErrors(cudaMemcpy(h_output.data(), d_output, BINS * sizeof(int), cudaMemcpyDeviceToHost));

  int total = 0;
  std::ofstream output_file;
  output_file.open("histogram_thread_coarsening.txt", std::ios::out);
  for (auto i: h_output){
    std::cout << i << "\n";
    output_file << i << "\n";
    total += i;
