...
int main(){
  ...
  checkCudaErrors(cudaMemcpy(da, ha.data(), sizeof(int) * (n*n), cudaMemcpyHostToDevice));
  checkCudaErrors(cudaMemcpy(db, hb.data(), sizeof(int) * (n*n), cudaMemcpyHostToDevice));
  matmul <<< grids, blocks >>> (da, db, dc);
  checkCudaErrors(cudaMemcpy(hc.data(), dc, sizeof(int) * (n*n), cudaMemcpyDeviceToHost));
